...
We then using the Q-learning algorithm implemented in the {Reinforcement Learning} package[@ReinforcementLearning], with the *standard default* parameters ($\alpha=.1,\gamma=.1, \epsilon=.1$).

## Testing the Model

We can now test the model by asking it to play some boards where we know the correct play.
For example $S_{32}$ where there is a scoring move.

```{r}
# Read from cache
model <- readr::read_rds(here::here("inst/models/q-learning-3x3.rds"))
predict(model,S[32])
```
...
On the other hand $S_{4}$ does not have a scoring move and should be passed.
```{r}
predict(model,c(S[4],S[6]))
```
...
So, equivalent of C2xC1, and one of the strongest moves available. However, we cannot be sure whether this was a random choice among scoring moves or the result of a strategic play. We can check this with another board.
...
```
We see that the model prefers _Pass_ to the correct move. So, while Gatai is capable of tactical moves, it doesn't seem to have learned any strategy. 

```{r}
#| label: fig-board-space-259  
#| layout-ncol: 3
#| fig-cap: "Correct Strategy"
#| fig-subcap: ["Board","Move 1","Move 2"]
#| fig-asp: 1
#| fig-width: 3
#| echo: false
p <- plot(s)
...
To overcome this issue, we may adjust the $\gamma$ parameter and retrain the model. This hyperparameter lets the algorithm know how far into the future to look.


```{r}
# Get from cache
gatai_2 <- readr::read_rds(here::here("inst/models/q-learning-3x3-adjusted.rds"))
predict(gatai_2,as.character(s))
```
So with this adjustment we have allowed Gatai to make strategic moves as well.

```{r}
#| label: fig-board-space-259-adjusted-model  
#| layout-ncol: 2
#| fig-cap: "After Gamma-adjustment: Gatai Thinks Ahead"
