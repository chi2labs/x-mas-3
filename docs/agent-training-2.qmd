...
This list contains several spaces which already feature a win. These should be removed, as they are not relevant for our purposes, i.e. they would never be tendered to the agent since once a score is made, the scoring tiles are removed from the board.

So therefore we have:

```{r}
my_scores <- purrr::map(S[1:2],~{
  cat(.x)
  # (score_binary_board(.x[[1]])>0)
})
...
```{r}
length(S) # 350K spaces
```

Which should be quite manageable.

Note: s\[124\] has a correct move and a superior tactic (or two correct moves)
